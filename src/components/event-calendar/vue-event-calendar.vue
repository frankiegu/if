...
export default {
  ...
  methods: {
    ...mapMutations(['UPDATE_CURRENT_DATE','UPDATE_MONTH']),
    handleChangeCurDay (date) {
      this.UPDATE_CURRENT_DATE(date)
      this.$emit('cur-date-changed', date)
  ...
  watch: {
    calendarOptions:{
      handler(val){
        let date = val.params.curYear+'-'+(val.params.curMonth+1)
        this.UPDATE_MONTH(date)
      },
      deep:true
    },
